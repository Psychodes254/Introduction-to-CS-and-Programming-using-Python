...
def get_letter_frequencies(word):
    ...
    """
    # Get the two functions for text to list and letter frequencies
    word_to_list = text_to_list(word)
    
    letter_freq = get_frequencies(word_to_list)
...
def calculate_similarity_score(freq_dict1, freq_dict2):
    ...
    # Calculate DIFF (sum of absolute differences)
    DIFF = 0
    for key in unique_elem:
        freq1 = freq_dict1.get(key, 0)
        freq2 = freq_dict2.get(key, 0)
        
        DIFF += abs(freq1 - freq2)
    
    # Calculate ALL (sum of all frequencies)
    ...
    # Return rounded similarity by 2 decimals
...
def get_most_frequent_words(freq_dict1, freq_dict2):
    ...
    """
    # Initialize an empty dict, then merge the two dict 
    combined_keys = {}
    ...
    max_freq_key = max(combined_keys.values())
    
    # All words whose combined frequency equals the highest frequency
    new_list = [key for key, value in combined_keys.items() if value == max_freq_key]
    
    # Return an alphabetically ordered list
...
def get_tfidf(tf_file_path, idf_file_paths):
    ...
    for word, tf_value in tf_dict.items():
        tf_idf[word] = tf_value * idf_dict[word]
    
    # Convert to list of tuples
    tfidf_dict = list(tf_idf.items())
    
    # Sort it correctly
    sorted_list = sorted(tfidf_dict, key=lambda x: (x[1], x[0]))
