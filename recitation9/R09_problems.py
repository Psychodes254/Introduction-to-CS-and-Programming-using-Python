...
class SwimWorkout(Workout):
    ...
    def __str__(self):
         ...
         cal_str = f"{round(self.get_calories(),1)}"
         retstr += f"| {cal_str} Calories {' '*(width-len(cal_str)-11)}|\n"
         num_laps = f"{self.numlaps}"
         retstr += f"| {num_laps} Num Laps {' '*(width-len(num_laps)-11)}|\n"
         avg_lap = f"{(self.get_average_laptime())}"
         retstr += f"| {avg_lap} Avg Lap {' '*(width-len(avg_lap)-11)}|\n"
...
swim_wrkt = SwimWorkout('9/30/2021 2:20 PM', '9/30/2021 2:50 PM')

for i in range(40):
    swim_wrkt.count_lap()
    
print(swim_wrkt.get_average_laptime())

swim_wrkt.end_swim_workout()